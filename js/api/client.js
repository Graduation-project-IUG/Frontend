...
(function () {
  ...
  async function apiFetch(path, options = {}) {
    ...
    if ((response.status === 401 || response.status === 403) && UNSAFE_METHODS.has(method)) {
      sessionStorage.removeItem("csrfToken");
    }

    if (!response.ok) {
      const message = Array.isArray(data?.message)
        ? data.message.map((item) => item.message || JSON.stringify(item)).join("\n")
        : data?.message || `فشل الطلب: ${response.status}`;
